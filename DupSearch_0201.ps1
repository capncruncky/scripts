...
foreach($file in $fileCollection)
{
    ...
    foreach($matchEntry in $MatchingFiles)
    {
        ...
        {
            #Write Output...
            Add-Content -Path $path/DupSearchLog.txt "`nFile Names and Lengths Match! Evaluating Hashes...`n*$($file.FullName)`n*$($matchEntry.FullName)" -PassThru
            ...
            {
                #Write Output...
                Add-Content -Path $path/DupSearchLog.txt "`n`t`tDUPLICATE FOUND!: $($file.Name)`n" -PassThru
                Add-Content -Path $path/DupSearchLog.txt "`t$($file.FullName)`n`t$($fileHash)`n`t$($matchEntry.FullName)`n`t$($matchEntryHash)`n" -PassThru
            ...
            {
                #Write Output...
                Add-Content -Path $path/DupSearchLog.txt "`n`t`tHashes are not identical. Moving on...`n" -PassThru
            }
        }
    }
    #Increment Counter...
...
$FinalResults | Format-List -GroupBy File_Hash | Out-File -FilePath $path/DupSearchLog.txt -Append -Encoding utf8
